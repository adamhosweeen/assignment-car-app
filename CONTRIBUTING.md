...
| **insights** | `insights/` | `insights/` | `insights/` | `insights/market_insights` | `car_popularity` (§11) + `supabase/functions/market-insights/` | CLAUDE.md §2 |
...
The database is **one file**, plus one Edge Function that fills a table:

```
supabase/
  migrations/0001_schema.sql   ← the whole schema: tables, RLS, functions,
                                  triggers, realtime, storage. FULL RESET.
  functions/market-insights/   ← fills car_popularity from data.gov.my,
    index.ts                      monthly. There is no seed file.
```
...
that each `create or replace`d the last — was collapsed into one; git history
has it if you ever need to see how something evolved.)
